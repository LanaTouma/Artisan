...
using Artisan.Web.Startup;
using AngleSharp.Html.Dom;
using AngleSharp.Html.Parser;
using Microsoft.AspNetCore.Hosting;
using Shouldly;

namespace Artisan.Web.Tests
{
    public abstract class ArtisanWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static ArtisanWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(ArtisanWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<ArtisanDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<ArtisanDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<ArtisanDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<ArtisanDbContext, Task<T>> func)
        {
            T result;
